...
- **[2026-07-21 방향 확정] 의제 뷰**: 별도 페이지 `/agenda`(§6.9, 헤더 네비 "의제") — 기간 토글(4주/3개월/6개월, 기준일=최신 회의일) × 분야 칩(룰 기반 7±1개 + **미분류 노출 필수**) × "이 기간의 흐름" AI 브리프(AiLabel 고정) × 의제 목록(의제문 title + 시드 지시 원문 인용 PlayLink + 최근 응답 PlayLink). 데이터 계약: SPEC-PIPELINE.md §2.5(`data/index/agenda.json`, `data/fields.json`). 설계 의도는 역할 분리다: 분야 칩은 결정적 룰(탐색 안정성), 흐름 브리프는 매 실행 재생성(의제 변화 추종) — 룰의 경직성과 AI의 불안정성이 서로를 보완한다. 국정과제 축과 별개 진입점이다(돌발 의제는 123과제에 안 잡힘). (처음 홈 섹션으로 구현했으나 2026-07-21 사용자 결정으로 별도 페이지 이동 — 홈은 영상 그리드 중심 유지.)
...
- 헤더(전역, layout): 로고("온말록" 워드마크 + red 닷 `.`) + 통합 검색바(단일 진입점 — 히어로 등 중복 검색바 금지, 아이콘은 SVG) + LIVE 배지(당일 생중계 있을 때만). 헤더 안에 텍스트 네비를 두지 않는다 [2026-07-21 확정: 開 마크 폐기, 홈 히어로 섹션 폐기]
- 네비(전역, layout): 유튜브 미니 가이드 문법 — 데스크톱(≥1024px) 좌측 고정 미니 사이드바(아이콘+라벨 세로 스택: 홈·의제·국정과제, 이후 오픈데이터 추가), 모바일 하단 탭바. 콘텐츠는 사이드바 폭(`--sidenav-w`)만큼 오프셋
- 상태 라인: 그리드 위 1줄, 무채색(`--dim`) — "최신 반영 · {kind} {날짜} · 회의 N건 · 발언 M문장" (green 금지 — §5.2에서 green은 결과·검증 전용). 클릭 시 최신 회의 /watch 이동. "모든 발언을, 원문 그대로." 카피는 푸터 첫 줄로 이동
...
### 6.9 ⑩ 의제 `/agenda` — "지금 활발한 의제"

- 헤더 네비 "의제"로 진입. agenda.json 소비, 기간·분야 필터는 클라이언트 계산
- 기간 토글: 4주(기본)/3개월/6개월. 기준일은 agenda.json `ref_date`(최신 회의일, `node_dates` 기준 집계)
- 분야 칩(단일 토글): 전체 + fields + **미분류(점선 테두리)**. 미분류는 숨기지 않는다 — 미분류 증가가 fields.json 갱신 신호
- "이 기간의 흐름" 카드: briefs에서 해당 기간 항목 렌더. **AiLabel("AI 요약 · 원문 대조 필요") 고정**, 항목마다 관련 스레드 PlayLink(§9-3 종착 충족)
- 의제 목록(기간 내 노드 수 내림차순, 상위 20): 행 = title(의제문, sans) + "AI 요약" 라벨 + 분야 칩 + 시드 지시 날짜 red 배지 + 시드 인용(명조 + PlayLink) + 최근 응답(날짜·발언자 "AI 추정" 병기 + PlayLink) + "발언 N건". 노드 1개 스레드는 최근 응답 줄 생략(시드와 중복). 행 클릭 → `/threads/[id]` (Phase 3)
...
- [ ] 의제 페이지 `/agenda`(§6.9) — agenda.json 소비, 기간·분야 필터는 클라이언트 계산 (데이터: SPEC-PIPELINE Phase P7)
